package com.example.spring_boot_webflux_apirest.controllers;
...
public class ProductoController {
    ...
    public Mono<ResponseEntity<Producto>> upload(@PathVariable String id, @RequestPart FilePart file){
        return service.findById(id).flatMap(p -> {
            p.setFoto(UUID.randomUUID() + "-" + file.filename().trim().replace(":","").replace("\\",""));
            return file.transferTo(new File("C://SpringWebFlux//uploads//" + p.getFoto())).then(service.save(p));
