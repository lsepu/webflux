package com.example.spring_boot_webflux_apirest.handler;
...
public class ProductoHandler {
    ...
    public Mono<ServerResponse> ver(ServerRequest request){
        ...
        return service.findById(id).flatMap(p -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromObject(p)))
    ...
    public Mono<ServerResponse> editar(ServerRequest request){
        ...
        return productoDb.zipWith(producto, (db,req) -> {
            ...
            return db;
        }).flatMap(p -> ServerResponse.created(URI.create("/api/v2/productos/" +p.getId())).contentType(MediaType.APPLICATION_JSON)
